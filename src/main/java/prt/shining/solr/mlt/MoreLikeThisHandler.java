...
public class MoreLikeThisHandler extends RequestHandlerBase {
    ...
    public void handleRequestBody(SolrQueryRequest req, SolrQueryResponse rsp) throws Exception {
        ...
        try {
            ...
            try {
                ...
                if (reader != null) {
                    ...
                    if (iterator.hasNext()) {
                        ...
                        mltDocs = mlt.getMoreLikeThis(parser, id, sortSpec.getSort(), start, rows, filters, interesting, flags);
                    }
                } else {
                    throw new SolrException(SolrException.ErrorCode.BAD_REQUEST,
            ...
            // TODO resolve duplicated code with DebugComponent.  Perhaps it should be added to doStandardDebug? just modify mlt.getRealBoostFuncMLTQuery()
            if (dbg == true) {
                try {
                    NamedList<Object> dbgInfo = SolrPluginUtils
                            .doStandardDebug(req, q, mlt.getRealBoostFuncMLTQuery(), mltDocs.docList, dbgQuery, dbgResults);
                    if (null != dbgInfo) {
                        if (null != filters) {
    ...
    public static class MoreLikeThisHelper {
        ...
        public MoreLikeThisHelper(SolrParams params, SolrIndexSearcher searcher) {
            ...
            this.boostFunction = params.get("mlt.bf");
        ...
        private Query realBoostFuncMLTQuery;
        ...
        public Query getRealBoostFuncMLTQuery() {
            return realBoostFuncMLTQuery;
        }
        ...
        /**
         * @param parser parser
         * @param q      Query to add resulting function query
         * @return Boosted query
         */
        public Query boosted(QParser parser, String boostFunction, Query q) throws SyntaxError {
            log.info("boost function is: " + boostFunction);
            if (boostFunction == null) {
                return q;
            }

            DoubleValuesSource dvs;
            Query bq = parser.subQuery(boostFunction, FunctionQParserPlugin.NAME).getQuery();
            if (bq instanceof FunctionQuery) {
                dvs = DoubleValuesSource.fromQuery(bq);
            } else {
                dvs = DoubleValuesSource.constant(0.0f);
            }

            return FunctionScoreQuery.boostByValue(q, dvs);
        }

        public DocListAndSet getMoreLikeThis(QParser parser, int id, Sort lsort, int start, int rows, List<Query> filters,
                                             List<InterestingTerm> terms, int flags) throws IOException {
            Document doc = reader.document(id);
            this.rawMLTQuery = mlt.like(id);
            this.boostedMLTQuery = getBoostedQuery(rawMLTQuery);
            if (terms != null) {
                fillInterestingTermsFromMLTQuery(rawMLTQuery, terms);
            }

            BooleanQuery.Builder realMLTQueryBuilder = new BooleanQuery.Builder();
            realMLTQueryBuilder.add(boostedMLTQuery, BooleanClause.Occur.MUST);
            // exclude current document from results
            realMLTQueryBuilder.add(
            ...
            this.realMLTQuery = realMLTQueryBuilder.build();

            // boost query by given query function, if possible
            try {
                realBoostFuncMLTQuery = boosted(parser, boostFunction, realMLTQuery);
            } catch (Exception e) {
                // no boosting
                realBoostFuncMLTQuery = realMLTQuery;
            }

            DocListAndSet results = new DocListAndSet();
            if (this.needDocSet) {
                results = searcher.getDocListAndSet(realBoostFuncMLTQuery, filters, lsort, start, rows, flags);
            } else {
                results.docList = searcher.getDocList(realBoostFuncMLTQuery, filters, lsort, start, rows, flags);
            }
            return results;
        }

        public DocListAndSet getMoreLikeThis(Reader reader, Sort lsort, int start, int rows, List<Query> filters,
                                             List<InterestingTerm> terms, int flags) throws IOException {
            ...
            realBoostFuncMLTQuery = boostedMLTQuery;
            if (terms != null) {
            ...
            if (this.needDocSet) {
                ...
                results.docList = searcher
                        .getDocList(boostedMLTQuery, filters, lsort, start,
                                rows, flags);
            }
            return results;
        }
    ...
    public String getDescription() {
        return "prt.shining.solr.mlt MoreLikeThis";
    }
